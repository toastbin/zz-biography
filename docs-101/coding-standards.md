...
- 单个文件有效行数不超过 **400 行**
- `.vue` 文件的有效行数**不计入 `<style>` 块**（CSS 不代表逻辑复杂度）
- 超出限制时使用 `/split-check <file>` skill 获取具体拆分建议

### 拆分原则

- 每个提取的单元必须有**清晰的单一职责**，不为凑行数拆分
- 子组件放 `components/` 子目录，composable 放 `composables/` 子目录
- 纯工具函数放 `utils/` 或 `lib/`
- 拆分只做结构性重组，不改变业务逻辑，不修改对外接口
